...
#define MIN_DEMAG_TIME                     ((uint16_t)0) /*!< Demagnetization delay in number of HF timer periods elapsed before a first
                                                           BEMF ADC measurement is processed in an attempt to detect the BEMF zero crossing */
#define SPEED_THRESHOLD_DEMAG              ((uint32_t)5000) /*!< Speed threshold above which the  RUN_DEMAGN_DELAY_MIN is applied */
...
/******************************   On The Fly startUp   **********************/
#define M1_OTF_MAX_BEMF_ERRORS             3 /* max inconsistent detected BEMF values before aborting the on the fly startup */
#define M1_OTF_LS_DETECT_PERC              2 /* low side MOS duty cycle during on the fly bemf detection phase  */
#define M1_OTF_LS_BRAKE_PERC               10 /* low side MOS duty cycle during on the fly braking phase */
#define M1_OTF_BEMF_THRESHOLD_PERC         10 /* threshold used to validate bemf transitions */
#define M1_OTF_SPEED_DUTY_CYCLE_RATIO      3
