...
static volatile uint16_t hBrakeCounterM1 = ((uint16_t)0);
static volatile uint16_t hOTFDetectionCounterM1 = ((uint16_t)0);
...
/* USER CODE END MC_Scheduler 0 */
    ...
    if(hOTFDetectionCounterM1 > 0U)
    {
      hOTFDetectionCounterM1--;
    }
    else
    {
      /* Nothing to do */
    }
    if(hBrakeCounterM1 > 0U)
    {
      hBrakeCounterM1--;
    }
    else
...
/**
  * @brief  It set a counter intended to be used for counting the delay required
  *         to brake the motor during 6step on-the fly startup.
  * @param  hTickCount number of ticks to be counted.
  * @retval void
  */
__weak void TSK_SetBrakeDelayM1(uint16_t hTickCount)
{
   hBrakeCounterM1 = hTickCount;
}

/**
  * @brief  It set a counter intended to be used for counting the delay required
  *         to detect the bemf during 6step on-the fly startup.
  * @param  hTickCount number of ticks to be counted.
  * @retval void
  */
__weak void TSK_SetOTFDetectionDelayM1(uint16_t hTickCount)
{
   hOTFDetectionCounterM1 = hTickCount;
}
...
/**
  * @brief  Use this function to know whether the time required to brake
  *         the motor during 6step on-the-fly startup has elapsed.
  * @param  none
  * @retval bool true if time has elapsed, false otherwise.
  */
__weak bool TSK_BrakeDelayHasElapsedM1(void)
{
  bool retVal = false;
  if (((uint16_t)0) == hBrakeCounterM1)
  {
    retVal = true;
  }
  return (retVal);
...
/**
  * @brief  Use this function to know whether the time required to detect
  *         bemf consistency during 6step on-the-fly startup.
  * @param  none
  * @retval bool true if time has elapsed, false otherwise.
  */
__weak bool TSK_OTFDetectionDelayHasElapsedM1(void)
{
  bool retVal = false;
  if (((uint16_t)0) == hOTFDetectionCounterM1)
  {
    retVal = true;
  }
  return (retVal);
